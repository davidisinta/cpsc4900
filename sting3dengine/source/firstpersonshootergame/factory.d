...
class SpawnFactory
{
    ...
    uint spawnChallengeProjectile(vec3 pos, vec3 scale, vec3 color, string urdfPath = "cube.urdf")
{
    uint eid = mEntityManager.create();
    ...
    // IMPORTANT: only challenge projectiles use "basic". Existing terrain/map/
    // tree/soldier objects must keep their own textured materials — passing
    // "basic" to them would draw them black (wrong vertex layout / no texture).
    IMaterial mat = mMaterials.get("basic");
    ISurface surface = new SurfaceTriangle(vbo);
    MeshNode node = new MeshNode("challenge_projectile_" ~ eid.to!string, surface, mat);

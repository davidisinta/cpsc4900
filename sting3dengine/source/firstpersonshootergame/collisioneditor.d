...
class CollisionEditor
{
    float[4][] mBoxes;
    string[] mBoxLabels;
    int[] mBoxEntityIds;   // -1 if not tied to an entity, otherwise the owning entity id
    ...
    /// Add a box that is bound to an entity. When the entity is destroyed,
    /// call removeBoxForEntity(eid) to drop both the draw-box and the
    /// camera collision box.
    void addBoxForEntity(uint eid, float minX, float minZ, float maxX, float maxZ, string label = "")
    {
        mBoxes ~= [minX, minZ, maxX, maxZ];
        if (label == "")
            mBoxLabels ~= "ent_" ~ eid.to!string;
        else
            mBoxLabels ~= label;
        mBoxSaved ~= false;
        mBoxEntityIds ~= cast(int)eid;
        mCamera.addCollisionBox(minX, minZ, maxX, maxZ);
    }

    /// Remove every box whose owning entity id matches `eid`.
    /// Returns the number of boxes removed.
    int removeBoxForEntity(uint eid)
    {
        int removed = 0;
        for (int i = cast(int)mBoxes.length - 1; i >= 0; --i)
        {
            if (mBoxEntityIds[i] == cast(int)eid)
            {
                deleteBox(i);
                removed++;
            }
    ...
    void deleteBox(int idx)
    {
        ...
        for (int i = idx; i < cast(int)mBoxes.length - 1; i++)
        {
            ...
            mBoxEntityIds[i] = mBoxEntityIds[i + 1];
        }
        mBoxes.length -= 1;
        mBoxLabels.length -= 1;
        mBoxSaved.length -= 1;
        mBoxEntityIds.length -= 1;
